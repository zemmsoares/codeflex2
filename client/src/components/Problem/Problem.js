...
function Problem() {
    ...
    function submitSubmission() {
        ...
        fetch(URL + '/submission', {
            ...
            window.resultsListener = setInterval(function () { fetchForResults(data); }, 1000);
        });

    }

    function fetchForResults(teste) {
        fetch(URL + '/api/database/Scoring/viewBySubmissionId/' + teste.id, { headers: { ...getAuthorization() } }).then(res => res.json())
            .then(data => {

                console.log(data)

                if (JSON.stringify(data) !== '[]' && teste.problem.testCases.length === data.length) {
                ...
                if (new Date().getTime() - window.secondsWaiting >= 30000) {
                    ...
                    console.log("Your evaluation is taking too long, please try again later.")
                    clearInterval(window.resultsListener);
